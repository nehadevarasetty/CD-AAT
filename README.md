...
bash brew install flex bison 

### Verify Installation

bash lex --version bison --version 
...
bash lex lexer.l bison -dy parser.y gcc y.tab.c lex.yy.c -o parser 

---

## Run the Program

bash ./parser test_input.c 
...
cmd win_flex --version win_bison --version 
...
cmd win_flex lexer.l win_bison -dy parser.y gcc y.tab.c lex.yy.c -o parser.exe 

---

## Run the Program

cmd parser.exe test_input.c 
...
text | Lexeme | Token Type | |---------|-------------| | int     | KEYWORD     | | main    | IDENTIFIER  | | =       | ASSIGN_OP   | 
